...
export class DemandeRVController {
  ...
  // GET /api/demandes/:id
  async getDemandeById(req: Request<{ id: string }>, res: Response, next: NextFunction) {
    try {
      const { id } = req.params;
      const demande = await this.demandeRVService.getDemandeById(id);
      res.status(200).json({
        success: true,
        data: demande,
      });
  ...
  // PUT /api/demandes/:id
  async updateDemande(req: Request<{ id: string }>, res: Response, next: NextFunction) {
    try {
      const { id } = req.params;
      const { date, heure, specialiteId, statutDemande, statutRV, commentaire } = req.body;

      const demande = await this.demandeRVService.updateDemande(id, {
  ...
  async deleteDemande(req: Request<{ id: string }>, res: Response, next: NextFunction) {
    try {
      const { id } = req.params;
      await this.demandeRVService.deleteDemande(id);
      res.status(204).json({
        success: true,
        data: null,
      });
  ...
  async cancelDemande(req: Request<{ id: string }>, res: Response, next: NextFunction) {
    try {
      const { id } = req.params;
      const demande = await this.demandeRVService.cancelDemande(id);
      res.status(200).json({
        success: true,
        data: demande,
      });
